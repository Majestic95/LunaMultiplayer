...
| **`/setvesselagency` on a vessel carrying kerbals** | Currently a NO-OP for crew. Under Stage 6 this becomes a real question: does the vessel's crew move into the new agency's roster? **Decision: yes**, the crew files migrate from `{old-agency}/Kerbals/{name}.txt` to `{new-agency}/Kerbals/{name}.txt` as part of the command, atomic rename. Same-name collision in target agency (target already has a "Jeb")? Refuse the command with operator-visible error. Documented at [SetVesselAgencyCommand.cs](../../Server/Command/Command/SetVesselAgencyCommand.cs). |
...
**Operator workflow:** archive `Universe/Kerbals/` then flip the gate. Or: opt into the allow flag and accept that all existing kerbals stay in the legacy shared dir (operator-visible Warning at boot listing the orphan kerbals; they remain readable under `Universe/Kerbals/` but no agency can mutate them — they become a frozen reference set).
...
| **6.7** | Re-audit `AgencyWolfCrewRouter` Slice E `BuildKerbalAgencyMap` against Stage 6 per-agency roster. Initial read: no change needed (it derives ownership from vessel `OwningAgencyId`, not from kerbal-file presence). Update Slice F `AgencyWolfMigration.CascadeOnDelete` to write the restored kerbal-file to the agency's subdir, not the legacy shared dir. | Extend `DeleteAgencyCommandWolfCascadeTest` for per-agency kerbal restoration. | Small |
| **6.8** | `/setvesselagency` crew migration: rename per-agency kerbal files atomically when a vessel changes hands. Refuse on same-name collision in target agency. Operator-visible Warning lists migrated kerbal names. | ServerTest: migration round-trip; collision-refuses; rollback-on-partial-failure. | Medium |
...
7. `/setvesselagency A→B` on a vessel carrying A's Jeb: Jeb's file migrates from `Agencies/A/Kerbals/Jebediah Kerman.txt` to `Agencies/B/Kerbals/Jebediah Kerman.txt` atomically. If B already has a "Jebediah Kerman", command refuses.
