...
**Phase 6 follow-up amendment (2026-05-20, v0.31.0-per-agency-private-9):** Q-Render's "scrub-foreign" assumption — that foreign-agency kerbal names don't resolve in the local `CrewRoster` because the per-agency `KerbalsRequest` filter prevented them from arriving — was wrong-by-design when same-name stock-4 kerbals collide across agencies (and per Q-Seed, they always do). The Phase 6.4 read filter only covers the `KerbalsRequest → KerbalReply` channel; foreign vessel `VesselProtoMsgData` payloads still relay cross-agency (intentionally, for visibility — Phase 6.6 labels them rather than hides them). When KSP's `ProtoVessel.Load` processes a foreign vessel's `crew = NAME` references, it resolves them against the local agency's same-named kerbals (e.g. foreign vessel's "Jebediah Kerman" → local agency's own Jeb), binding the local kerbal to the foreign vessel's seat and flipping its `rosterStatus` to Assigned. The post-Load `ScrubInvalidProtoCrew` (BUG-023 defense) does NOT catch this — it only scrubs `null` entries, and a resolved-but-foreign `ProtoCrewMember` is not null. Net effect: every agency permanently loses access to any kerbal name another agency has assigned to a vessel — every kerbal name becomes "first agency to fly it claims it for the cohort." Confirmed v8.1 soak: Agency A could only launch Jeb; B and C had assigned Bill / Val on flights, so A's local Bill / Val showed as Assigned to those foreign vessels and were unavailable in A's AC.

**Fix:** new pre-Load strip at `LmpClient/Systems/VesselProtoSys/VesselProto.cs::CreateProtoVessel` — when the incoming vessel is foreign under `PerAgencyKerbalRosterEnabled`, walk all `PART` child nodes and remove every `crew = NAME` value via `partNode.RemoveValues("crew")` BEFORE `CreateSafeProtoVesselFromConfigNode` runs. KSP's `ProtoVessel` ctor + `Load` then see the vessel as empty-seated → never look up the names → never bind local kerbals. The Phase 6.6 `ForeignCrewCount` registry is populated from the pre-strip count so the `"Crew: N (Agency)"` label still renders correctly. The post-Load `ScrubInvalidProtoCrew` in `VesselLoader` is unchanged for local-vessel BUG-023 handling but now gates its Phase 6.6 `ForeignCrewCount` mgmt on `!IsForeignVessel` (pre-Load owns the registry for foreign vessels). Net cost: foreign vessels appear empty-seated in tracking station / map / IVA-not-applicable views from non-owner agencies' clients — Phase 6.6 label is the only indicator of "someone is aboard." Acceptable, no UX regression vs original Q-Render commitment.

**Tradeoff vs Option 3 (server-side per-recipient strip) — re-evaluated and still REJECTED:** the same player outcome (foreign vessels empty-seated, label visible) is achieved client-side without the hot-path decompress/edit/recompress cost. Option 3 would additionally cover any future mod-side code path that scans vessel-proto bytes (e.g. a mod scanning `VesselProtoMsgData.RawData` on receive before LMP's `CreateProtoVessel` runs), but no such consumer exists today in MKS / WOLF / Final Frontier / Contract Configurator. Defer Option 3 to v3 if a future mod-compat finding shows the client-side strip isn't enough.

**Initial-connect race — pessimistic strip on ownership MISS/Empty (integration-lens Finding 1):** the server's vessel-proto relay path at [Server/Message/VesselMsgReader.cs:352](../../Server/Message/VesselMsgReader.cs#L352) forwards the ORIGINAL sender wire bytes per the [[5.18b relay-vs-store note]] — but KSP's `BackupVessel` / `protoVessel.Save` strips unknown top-level fields like `lmpOwningAgency` on every local-owner resend, so non-first relayed protos arrive at peers WITHOUT the stamp. If a peer's `CreateProtoVessel` runs against a relay-stripped proto BEFORE the authoritative `VesselSync` reply has populated `VesselOwnership[VesselId]`, the registry lookup misses or returns Empty — and the original `IsForeignVessel` check would return false (Empty short-circuit), letting the foreign crew names through to KSP's `ProtoVessel.Load` and re-creating the very pollution this fix prevents. **Resolution:** the strip predicate is composed via `AgencyMembership.ShouldStripForeignCrew` which returns true on registry MISS or Empty under combined gate=on (in addition to confirmed-foreign). The local player NEVER receives their OWN vessel's bytes back via relay (server excludes sender), so the pessimistic branch only fires for genuinely-foreign or Unassigned-sentinel vessels. Unassigned vessels (spec §10 Q3, pre-0.31 carry-over) also strip-clean by design — no agency legitimately "owns" them, so binding to anyone's local roster would be wrong. After `VesselSync` arrives and updates the registry, subsequent proto pass-throughs hit the confirmed-foreign or confirmed-local branch normally. Decision matrix pinned by `AgencyMembership.ShouldStripForeignCrew` LmpClientTest cases (gate-off / local-empty / confirmed-local / confirmed-foreign / MISS-pessimistic / Empty-pessimistic / null-dict-pessimistic / idempotency).

**Cross-agency-dock limitation (integration-lens Finding 7):** when Agency A's vessel docks with Agency B's foreign vessel, A's client receives B's proto with crew names already stripped by the pre-Load strip (B's vessel was foreign to A pre-dock). After the dock, `AgencyVesselCoupleReconciler` (server-side) picks an owning agency for the merged vessel. If A wins, the next merged-vessel proto carries B's original crew names (re-sourced from the server's canonical store via VesselSync), but A's `CreateProtoVessel` now sees the vessel as local — strip does NOT fire — and B's kerbal names get scrubbed by `ScrubInvalidProtoCrew` as null (B's kerbal files are not in A's local roster per Phase 6.4). Net: B's crew aboard the merged vessel are silently lost from the server-canonical proto on A's next BackupVessel resend. **Documented as known limitation for v1.** Cross-agency docking with crew is rare; spec §Q-CrossAgency contemplates the docking surface but leaves crew-handoff semantics under-specified. Two future-resolution paths (deferred): (a) refuse cross-agency docks at `AgencyVesselCoupleReconciler` when crew aboard the non-winner side; (b) extend the reconciler with a kerbal-handoff step that migrates the non-winner's crew files to the winner's per-agency subdir before the merge proto re-broadcasts (mirrors Phase 6.8's `/setvesselagency` crew migration shape). Operator workaround for v1: EVA crew off the foreign vessel before docking, recover them separately on their own vessel.

**Recovery on upgrade from v8.1 → v9 (consumer-lens DOC/SPEC NEEDED #5):** existing v8.1 playtest universes have local-CrewRoster pollution baked into client save games (Agency A's local Bill marked `rosterStatus=Assigned` to Agency B's vessel because of the original bug). The fix is forward-looking only — it prevents NEW pollution, doesn't reach into existing client-side save games to repair them. **However, self-heal happens automatically** via periodic KerbalProto resyncs from the server: under per-agency routing, each agency's KerbalProto for their own kerbal overwrites the local `rosterStatus + seat + seatIdx` fields (see [LmpClient/Systems/KerbalSys/KerbalSystem.cs:280-295](../../LmpClient/Systems/KerbalSys/KerbalSystem.cs#L280-L295)), restoring A's Bill to Available after A's next session-start when B's stale pointer is replaced by the server's authoritative state. No operator action required; players may see the polluted state briefly on first reconnect after upgrade, then it self-clears within a few seconds.
...
**Why same names:** simplest. Each client only ever sees its own agency's `CrewRoster` via the `KerbalsRequest` filter. Audit §10 Risk 1 (same-name behaviour in KSP's `CrewRoster.Crew`) is sidestepped at the kerbal-channel level.

**Phase 6 follow-up correction (2026-05-20, v0.31.0-per-agency-private-9):** the earlier "collisions across rosters never materialise" claim above was true for the `KerbalsRequest → KerbalReply` channel only. Foreign-agency `VesselProtoMsgData` payloads relay cross-agency (intentionally — for visibility, per Q-Render), and KSP's `ProtoVessel.Load` resolves the foreign vessel's `crew = NAME` entries against the local `CrewRoster`, binding the local agency's same-named kerbal to the foreign vessel's seat. See Q-Render's "Phase 6 follow-up amendment" above for the failure mode + the pre-Load strip fix landed in `LmpClient/Systems/VesselProtoSys/VesselProto.cs::CreateProtoVessel`. With the fix, the original Q-Seed sign-off is preserved: each agency has independent kerbal copies that diverge cleanly from first action, even though they start with identical names.
...
| **Same-name kerbal collision posture between `/deleteagency` and `/setvesselagency`** (v8.1 audit cross-phase (d) clarification) | These two commands deliberately handle same-name collisions DIFFERENTLY because the problem statements differ: **`/deleteagency`** is a destructive, in-progress operation (the source agency is being wiped) — refusing the WHOLE cascade because one passenger collides would strand the other passengers. Phase 6.7's per-kerbal Collision DROP preserves destination's existing kerbal + skips the source's version, counted in `CollidedKerbalNames`, separate from `FailedKerbalNames`. **`/setvesselagency`** is a reversible reassignment with no in-progress destruction — refusing the WHOLE command lets the operator rename or pick a different destination and retry. The asymmetry is intentional and operator-visible via distinct `result=` codes (`result=refused-collision` in 6.8 vs per-name `CollidedKerbalNames` in 6.7). |
...
- **Final Frontier per-agency ribbon partition** (v8.1 audit Mod-compat M1 disposition). Phase 6.8's wire-pushed `KerbalProtoMsgData` to a destination client triggers KSP's `KerbalRoster.AddCrewMember` → `GameEvents.onKerbalAdded` on the destination client side. Final Frontier subscribes to `onKerbalAdded` to mint a fresh ribbon record; under per-agency mode, this would silently fork the kerbal's award history across agency ownership transitions. **Disposition for v8.1:** documented as expected, no client-side suppression mechanism shipped because (a) the Phase 6.8 wire-push uses the same `KerbalProtoMsgData` shape as a periodic resync, so there's no in-band signal distinguishing "fresh migration" from "regular resync" for the dest client to gate on; (b) the per-agency Final Frontier integration is itself Phase 6.9 territory; (c) no current shipped mod observably breaks. **Fix when Phase 6.9 lands:** either add a new wire-message type (`KerbalMigrationProtoMsgData`) distinguishing migration intent so the dest client can set a one-shot `IgnoreOnKerbalAdded` flag like `ShareContractsSystem.IgnoreEvents`, OR accept the once-per-migration ribbon-mint reset as a documented quirk of operator-driven reassignment.
- **Kerbal-file content tampering protection** (v8.1 audit security SHOULD FIX disposition). A modified client crafting a `KerbalProtoMsgData` for its own roster's kerbal can write forged careerLog bytes (`experience=999999`, fake flightLog timestamps, etc.) verbatim to the per-agency subdir. Phase 6.9-hardening's `KerbalNameValidator` covers the kerbal-NAME attack surface but does NOT range-check the kerbal-DATA bytes. Under per-agency gate=on the cheat is self-contained (the cheater's own agency); the cross-leak vector is the Phase 6.7 `--restore-to <agency>` cascade and Phase 6.8 `/setvesselagency` migration, both of which propagate the byte-blob to a different agency. **Disposition for v8.1:** documented as accepted risk for private trusted cohort. **Fix when adversarial cohort emerges:** range-check `ProtoCrewMember.experience` + validate state enum + cap flightLog entry count at the wire boundary in a new `KerbalProtoValidator.IsValid(byte[] bytes, out string reason)` helper, hooked into `KerbalMsgReader.HandleMessage` alongside the existing `KerbalNameValidator`.
