...
| `FileHandler.ReadAtomic` logs a `LunaLog.Warning` every time it recovers from `.bak`. If `AgencySystem` reads on every operation rather than caching the in-memory state, the warning spams the log until the canonical path is healed. | Stage 5.14c session 12, pass-1 review-agent | Caller-side concern, not a `FileHandler` design issue. The fix is in the eventual `AgencySystem` (5.15a): on first read that surfaces `.bak`, immediately `WriteAtomic(path, recoveredContent)` to heal the canonical path, then `AgencySystem` holds the state in-memory until the next mutation. | Stage 5.15a (`AgencySystem` lifecycle). |
