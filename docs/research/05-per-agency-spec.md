...
| Facility destructibles (damage) | **Shared by design** (Stage 5.17e-7 sign-off) | Building damage / repair state is a physical-world property of the shared KSC, not a per-agency career metric. If Alice crashes into the VAB, every player sees the destroyed VAB until repaired — same as shared-agency mode. Per-agency damage state would require N coincident physical KSCs (the contradiction the facility-upgrade-tier decision already rejected). `FacilityCollapse` / `FacilityRepair` messages stay on the legacy `FacilityMsgReader` + `ScenarioDestructiblesDataUpdater` path under `PerAgencyCareer=true`; no router gate. Operator note: a repair cost charged via the existing legacy path (if KSP applies one) hits whichever agency triggers the repair, since the repair-cost path flows through `ShareFundsSystem` which 5.17e-3 routes per-agency. Acceptable — repair is rare and the operator can `setagencyfunds` if a single agency disproportionately pays for shared building damage. |
...
### Resolved (signed off 2026-05-17, session 15 audit — band-1 design checks)

The session-15 audit ([`05b-ksp-career-surface-audit.md`](05b-ksp-career-surface-audit.md) + [`05c-per-agency-completeness-checklist.md`](05c-per-agency-completeness-checklist.md) + [`05d-per-agency-path-forward.md`](05d-per-agency-path-forward.md)) surfaced five product decisions that needed pre-coding resolution before band-1 implementation could land safely. Decisions captured here and reflected in `AgencySystem.PerAgencyEnabled` (the combined gate) + the per-surface routers + the K1 kerbal guard + the boot refusal hardening.

- **Q-Mode — Game mode scope:** **Career-only.** `PerAgencyCareer=true` requires `GameMode=Career`. Science mode has no `Funding.Instance` on the client (per-agency mid-session writes would NRE); Sandbox has no career scalars to project at all. The combined gate (`AgencySystem.PerAgencyEnabled` — Stage 5.17e-1) folds both into one truth value; boot-time misconfig warning fires when set wrong; runtime per-agency code paths are no-ops under non-Career regardless of the setting. Operator workflow: pick Career mode before enabling.
- **Q-Kerbal — Kerbal roster:** **Shared roster (K1) with admin-guard against grief.** Kerbals remain in the single global `Universe/Kerbals/{name}.txt` namespace in v1; per-agency rosters deferred to Stage 6. Server-side guard refuses `KerbalRemove` from a player whose vessel doesn't own the kerbal being removed — prevents Bob despawning Alice's Jeb. This is a deliberate v1 limitation: full per-agency rosters require wire-extension on every `Kerbal*MsgData`, agency-aware request filtering, and original-kerbal name-disambiguation logic that doesn't fit in band-1 scope.
- **Q-VesselEconomy — Recovery / revert ownership:** **Vessel owner pays/receives non-contract economy; contract rewards go to contract owner (already Q4); mismatched recoverer is admin-reassigned.** Vessel-linked non-contract economy (recovery funds, recovery science, revert refund, termination penalty) routes to `lmpOwningAgency`. If a player tries to recover a vessel not owned by their agency, the server rejects with a "use transferagency or have owner recover" message (deferred to Stage 5.18d admin commands — band-2 work).
- **Q-BootRefusal — Misconfigured boot:** **Refuse boot by default** when `PerAgencyCareer=true` is enabled on a non-fresh universe (vessels exist, agencies don't). Operator can override with a new explicit `AllowEnablePerAgencyOnExistingUniverse=true` setting. The existing `WarnAbout*OnUpgrade` boot diagnostics (5.17c/d + 5.17e-4/5/6) already log loudly; the refusal flips them to hard fail-closed so misconfig can't silently corrupt the next first-connect. Stage 5.17e-9 implements.
- **Q-Destructibles — Facility damage / repair:** **Shared by design.** Building damage state belongs to the shared physical KSC (the same KSC every agency upgrades tiers on); per-agency damage would require N coincident physical KSCs (the contradiction the facility-tier-per-agency decision already rejected). `FacilityCollapse` / `FacilityRepair` remain on the legacy `FacilityMsgReader` + `ScenarioDestructiblesDataUpdater` path — NO router gate under `PerAgencyCareer=true`. See §2 Facility destructibles row (Stage 5.17e-7).

### Global server rules vs split agency state (Stage 5.17e-7 sign-off)

For clarity to future maintainers + the Stage 5.18a/b client author, the following server-level settings remain GLOBAL under `PerAgencyCareer=true` (i.e. shared across all agencies; configured once per server in `Settings/GameplaySettings.xml`):

- **Difficulty / economy multipliers**: `ReentryHeatScale`, `ResourceAbundance`, `FundsGainMultiplier`, `FundsLossMultiplier`, `RepGainMultiplier`, `RepLossMultiplier`, `RepLossDeclined`, `ScienceGainMultiplier`, `BuildingDamageMultiplier`.
- **Career rule toggles**: `AllowNegativeCurrency`, `BypassEntryPurchaseAfterResearch`, `CanRevert`, `MissingCrewsRespawn`, `AutoHireCrews`, `IndestructibleFacilities`, `AllowOtherLaunchSites`, `AllowStockVessels`.
- **Kerbal physics / progression**: `KerbalExp`, `ImmediateLevelUp`, `GKerbalLimits`, `KerbalGToleranceMult`, `GPartLimits`, `PressurePartLimits`, `RespawnTime`.
- **CommNet**: all CommNet settings are server-global per Q-CommNet sign-off (`CommNetwork`, `RequireSignalForControl`, `DsnModifier`, `RangeModifier`, `OcclusionModifierVac`, `OcclusionModifierAtm`, `ExtraGroundstations`, `ReentryBlackout`).
- **Parts policy**: `PartUpgrades`, `ActionGroupsAlways`, `EnableFullSASInSandbox`, `ObeyCrossfeedRules`.
- **Starting amounts** (`StartingFunds` / `StartingScience` / `StartingReputation`): server-global; each new agency seeds from these.

What IS per-agency: Funds (5.17e-3), Science (5.17e-3), Reputation (5.17e-3), Tech tree (5.17e-4), Science subjects (5.17e-5), Purchased parts (5.17e-5), Experimental parts (5.17e-5), Strategies (5.17e-6), Achievements / world firsts (5.17e-6), Facility upgrade tiers (5.17e-6), Contracts post-Accept (5.17d, Q6 hybrid — Offered stays shared).

### Inherited Luna limitations (Stage 5.17e-7 sign-off — NOT in band-1 scope)

Per-agency mode inherits these existing LMP limitations without attempting per-agency variants in v1:

- **Quickload disabled** (LMP-wide limitation; not per-agency-affecting).
- **Revert restrictions after vessel switch / spectating** (LMP-wide; per-agency revert refund routes via `ShareFundsSystem` which 5.17e-3 routes per-agency).
- **Tourist contracts unsupported** (LMP-wide; CC + stock tourist contracts disabled at the message level).
- **Rescue contracts unsupported** (LMP-wide; same surface as tourist).
- **Ground deployables limited** (LMP-wide; `DeployedScience` etc. — known limitation in the audit's "DLC and extra scenarios" section, ground-deployed vessels don't sync via the normal vessel-proto path).
