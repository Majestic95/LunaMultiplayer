...
- **Status:** Part A FIXED ON FORK (session 7) — server broadcasts `VesselPinned` for each lock-owned vessel before fanning out lock releases; remaining clients hold the named vessels immortal via `VesselPinnedSystem` until the original pilot reconnects or another player explicitly takes the helm. Part B (graceful-disconnect pre-flush proto with 2s ack wait) deferred. See [`docs/research/02-analysis/bug-010-disconnect-vessel-handoff.md`](02-analysis/bug-010-disconnect-vessel-handoff.md).
...
6. ~~**[BUG-010] Disconnect destroys craft within rendering distance of another player**~~ — ✅ PART A FIXED ON FORK (session 7). Server-broadcasts `VesselPinned`; remaining clients hold the leaver's vessels immortal via `VesselPinnedSys` until any player takes the helm. Part B graceful-disconnect proto-flush deferred. See [`docs/research/02-analysis/bug-010-disconnect-vessel-handoff.md`](02-analysis/bug-010-disconnect-vessel-handoff.md).
