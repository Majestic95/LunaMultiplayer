...
`LmpClientTest/` now (57 tests on `net472` via MSTest):
...
- **Stage 5 — Per-agency career (2–4 months, separate branch)** — IN PROGRESS on `feature/per-agency`. Spec at [docs/research/05-per-agency-spec.md](docs/research/05-per-agency-spec.md). Progress tracker at [docs/research/05a-stage5-progress.md](docs/research/05a-stage5-progress.md).
  ...
  - 5.17b (next on the server-write surface) Per-agency `Share*` routing (×13 files) — step 11. Write-path counterpart to 5.17c's read-side projection. Bracket every refund with `StartIgnoringEvents/StopIgnoringEvents` per BUG-025 precedent. Ships 7 of the remaining 8 deferred mutation wire messages.
  - 5.18c (next on the client surface) Client UI — `AgencyWindow`, `AgencyCreateWindow`, tracking-station agency overlay (consumes 5.18b's `VesselOwnership` registry), cross-agency lock-denial toast.
  - 5.18d Admin commands — `listagencies`, `setagency*`, `transferagency`, `deleteagency`. Also: recovery-economy ownership enforcement (vessel-owner check on Recover), and the force-full-sync-on-reconnect decision deferred from 5.18b. ContractSystem scenario projection deferred here too.
