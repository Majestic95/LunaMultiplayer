...
`MockClientTest/` (17 tests on `net10.0` via MSTest):
...
- `AgencyHandshakeTest` — five end-to-end agency-registration cases (Stage 5.16a, session 13): auto-registered handshake delivers AssignedAgencyId+State, second player's handshake summary includes the first, CreateRequest("valid") persists to disk via the owner-only channel, CreateRequest("") rejects with Guid.Empty + non-empty Reason, PerAgencyCareer=false produces zero Agency traffic on handshake (dual-mode silence)
...
- **Stage 5 — Per-agency career (2–4 months, separate branch)** — IN PROGRESS on `feature/per-agency`. Spec at [docs/research/05-per-agency-spec.md](docs/research/05-per-agency-spec.md). Progress tracker at [docs/research/05a-stage5-progress.md](docs/research/05a-stage5-progress.md).
  ...
  - 5.16b (next) `lmpOwningAgency` on vessels (spec §12 step 7).
