...
`ServerTest/` (86 tests on `net10.0` via MSTest):
...
`MockClientTest/` (10 tests on `net10.0` via MSTest, Stage 4.9 v1 + 4.10):
...
- `Bug005SubspaceRejectTest` — end-to-end BUG-005/006 coverage: two mock clients in different subspaces, a planted vessel in `VesselStoreSystem.CurrentVessels`. Past-subspace client → future-auth vessel proto-update is rejected (stored vessel reference unchanged, no relay to the future client); future-subspace client → past-auth vessel is accepted (relay reaches the past client). Third case (retro-review M5) covers the widened gate: past-subspace `VesselPositionMsgData` is rejected the same way, proving the `RejectIfPastSubspace` helper catches non-Proto vessel relays. Sample vessel ConfigNode is loaded from `ServerTest/XmlExampleFiles/Others/` via a relative-path walk-up — no fixture duplication.
- `Bug010PinnedBroadcastTest` — end-to-end BUG-010 Part A coverage: two mock clients, the leaving client plants Control + Update locks on two distinct vessel ids (via direct `LockSystem.AcquireLock`), then disconnects. The watcher must receive one `VesselPinnedMsgData` per locked vessel with `AbsentPlayerName` = the leaver and a non-empty `Reason`. Second case asserts a peer that disconnects BEFORE handshake emits no pin broadcast (the `if (client.Authenticated)` gate in `ClientConnectionHandler.DisconnectClient`).
...
- **Mock-client test harness:** Stage 4.9 v1 shipped (session 4); Stage 4.11 CI integration shipped session 8 (the existing `build.yml` workflow now includes `MockClientTest` in its matrix and runs on push-to-master as well as PRs). `MockClientTest` brings the real Server up in-process on a free UDP port; `MockNetClient` completes the LMP handshake against it. Stage 4.10 server-observable regression tests complete (BUG-001 / BUG-005/006 / BUG-051a / BUG-010 Part A); client-internal regression tests for BUG-003/004 + BUG-051b still pending in `LmpClientTest/` (helpers need extraction first).
...
- **Bug inventory:** see `docs/research/01-bug-inventory.md` for the full 51-bug catalogue (BUG-051 added during Phase-2). Stage 2 closed the top-1 priority plus four others: BUG-001 (solo-subspace), BUG-003/004 (interp cap), BUG-005/006 (cross-subspace lock), BUG-014 (audit-closed via upstream PR #628), BUG-051 (stuck warp). Stage 3 picked up BUG-013 (reaction-wheel stateString sanitiser), BUG-008 Phase A (PQS-timing realignment), BUG-045 (Breaking Ground deployable science — Strategy B.1), and BUG-010 Parts A + B (disconnect-driven vessel pinning + graceful-disconnect proto-flush, session 7); BUG-018 / 019 / 024 audit-closed via upstream PR #687. Remaining top-10: BUG-023 (astronaut complex desync), BUG-025 (R&D double-purchase), BUG-033 (backup race).
...
- **Stage 3 — Operational tooling (3–4 weeks, parallel)** — IN PROGRESS
  ...
  - ⏳ 3.8 Phase-2 + fixes for remaining top-10 — IN PROGRESS. ✅ BUG-013 fix shipped (`c5ab8fa5`, reaction-wheel `stateString` sanitiser). ✅ BUG-008 Phase A shipped (session 6, `PqsAlignmentRoutine` + `VesselProtoSystem` hook) — Phase B (server-side stored `terrainAltitude`) deferred until Phase A soaks. ✅ BUG-018 / BUG-019 / BUG-024 audit-closed via upstream PR #687. ✅ BUG-045 shipped (Strategy B.1). ✅ BUG-010 Part A + Part B shipped (session 7 — server-broadcasts `VesselPinned` + client immortal-hold via `VesselPinnedSys`; clean-disconnect proto-flush in `MainSystem.DisconnectFromGame` for the dock-then-undock pose case). ✅ BUG-033 shipped (session 8, `87105f41` — `BackupScenarios` now serializes each `ConfigNode` under the matching per-scenario writer lock via `ScenarioDataUpdater.GetSemaphore`). Remaining: BUG-023 / BUG-025.
- **Stage 4 — Mock-client test harness (2–4 weeks)** — IN PROGRESS
  ...
  - ✅ 4.10 Regression tests for shipped fixes — server-observable bugs COMPLETE. ✅ BUG-051a (`Bug051aDedupTest`). ✅ BUG-001 (`Bug001SoloBroadcastTest`). ✅ BUG-005/006 (`Bug005SubspaceRejectTest`). ✅ BUG-010 Part A (`Bug010PinnedBroadcastTest`). Also surfaced + fixed [BUG-052](docs/research/01-bug-inventory.md) (vendored Lidgren `NetReliableSenderChannel.DestoreMessage` NRE on late-ACK during peer shutdown) while writing the first test. BUG-003/004 + BUG-051b are client-internal and need a separate `LmpClientTest` net472 project that references the now-buildable `LmpClient.dll`.
